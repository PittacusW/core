...
class RememberTraitTest extends TestCase {
  ...
  : void {
    ...
    $this->assertSame('Alpha', RememberableModel::query()->value('name'));

    DB::table('rememberable_models')->whereKey($model->getKey())->update(['name' => 'Beta']);
    ...
    $this->assertSame('Gamma', RememberableModel::query()->value('name'));
  }
  ...
  : void {
    ...
    $this->assertSame(1, RememberableModel::query()->count());

    $model->delete();

    $this->assertSame(0, RememberableModel::query()->count());
  }
